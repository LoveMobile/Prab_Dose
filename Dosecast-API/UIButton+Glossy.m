...
static void perceptualCausticColorForColor(float* inputComponents, float* outputComponents)
{
  static const float CAUSTIC_FRACTION				= 0.35;
	...
	rgb_to_hsv((const float*)CGColorGetComponents([[UIColor yellowColor] CGColor]), temp);
    ...
    {
		rgb_to_hsv((const float *)CGColorGetComponents([[UIColor magentaColor] CGColor]), temp);
...
+ (void)setPathToRoundedRect:(CGRect)rect forInset:(NSUInteger)inset inContext:(CGContextRef)context
{
	...
	static const NSUInteger cornerRadius = 6;
...
- (void)setBackgroundToGlossyRectOfColor:(UIColor*)color withBorder:(BOOL)border forState:(UIControlState)state
{
    ...
    UIColor* edgeColor = [UIColor colorWithRed:r/2.0f green:g/2.0f blue:b/2.0f alpha:a];
...
- (void)setBackgroundToRectOfColor:(UIColor*)color forState:(UIControlState)state
{
    CGSize size = self.frame.size;
    UIGraphicsBeginImageContext(size);
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    ...
    UIBezierPath *pillShapePath = [UIBezierPath bezierPathWithRoundedRect:self.bounds cornerRadius:self.bounds.size.height/6];
